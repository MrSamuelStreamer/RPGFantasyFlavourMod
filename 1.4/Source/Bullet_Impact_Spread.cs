...
namespace VPE_Ranger
{
    ...
    public static class Bullet_Impact_Spread
    {
        private static void Postfix(ref Thing hitThing, Bullet __instance)
        {
            ...
            if (pawn.health.hediffSet.HasHediff(VPERanger_DefOf.MakaiRanger_BuffThree))
            {
                HediffComp_ArcingShot comp = pawn.health.hediffSet.GetFirstHediffOfDef(VPERanger_DefOf.MakaiRanger_BuffThree).TryGetComp<HediffComp_ArcingShot>();
                if (comp.shotCount > 0 && !comp.isBursted)
                {
                    ...
                    {
                        possibleTargetCell.Add(VPERangerUtility.RandomCellAroundCellBase(__instance.intendedTarget.Cell, -5, 5));
                    }
                    for (int i = 0; i < possibleTargetCell.Count; i++)
                    {
                        ...
                        {
                            shrapnelThing.Launch(__instance.Launcher, possibleBuilding, possibleBuilding, ProjectileHitFlags.IntendedTarget);
                        }
                        else
                        {
