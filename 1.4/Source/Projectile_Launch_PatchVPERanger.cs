	...
	[HarmonyPatch(typeof(Projectile), "Launch", new Type[]
{
	...
	public static class Projectile_Launch_PatchVPERanger
	{
		...
		{
			if (__instance == null || !(__instance.Launcher is Pawn instigator) || !instigator.health.hediffSet.HasHediff(VPERanger_DefOf.MakaiRanger_BuffThree))
			{
				return;
			}
			HediffComp_ArcingShot comp = instigator.health.hediffSet.GetFirstHediffOfDef(VPERanger_DefOf.MakaiRanger_BuffThree).TryGetComp<HediffComp_ArcingShot>();
